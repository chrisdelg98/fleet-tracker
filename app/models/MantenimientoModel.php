...
final class MantenimientoModel
{
    ...
    public function control(array $filtros): array
    {
        $sql = 'SELECT u.id, u.placa_unidad, u.marca, u.modelo, u.estado_vehiculo,
                       ...
                       COALESCE(pl.intervalo_km, pd.intervalo_km)     AS intervalo_km,
                       COALESCE(pl.intervalo_dias, pd.intervalo_dias) AS intervalo_dias,
                       COALESCE(pl.umbral_km, pd.umbral_km)           AS umbral_km,
                       COALESCE(pl.umbral_dias, pd.umbral_dias)       AS umbral_dias,
                       COALESCE(pl.nombre, pd.nombre)                 AS plan
                  FROM unidades u
                  JOIN estaciones e ON e.id = u.estacion_id
