...
$nC = 0;
foreach ($categorias as [$nombre, $esFlota, $esMotriz, $admiteArrastre, $orden]) {
    $nC += $insertIfMissing(
        $pdo,
        'SELECT 1 FROM categorias_vehiculo WHERE nombre = ?', [$nombre],
        'INSERT INTO categorias_vehiculo (nombre, es_flota_operativa, es_motriz, admite_arrastre, orden) VALUES (?, ?, ?, ?, ?)',
        [$nombre, $esFlota, $esMotriz, $admiteArrastre, $orden]
    );
}
